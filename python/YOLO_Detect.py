...
def main():
    ...
    if opt.input.lower() != 'camera' and opt.input != '/dev/video0':
        logger.info("视频输入: 仅实时显示处理结果，不保存到文件。")

    can_show = bool(os.environ.get('DISPLAY'))
    if can_show:
        try:
            cv2.namedWindow("RDK YOLO", cv2.WINDOW_NORMAL)
        except Exception:
            can_show = False
    if not can_show:
        logger.warning("未检测到可用的显示环境，将跳过实时显示。")
    ...
    while True:
        ...
        if can_show:
            cv2.imshow("RDK YOLO", frame)
            if cv2.waitKey(1) & 0xFF == ord('q'):
                logger.info("检测到退出指令，提前结束处理。")
                break

...
try:
    ...
    _HAS_CYTHON_BBOX = True
except Exception:
    _HAS_CYTHON_BBOX = False
    def bbox_ious(boxes_a, boxes_b):
...
def linear_assignment(cost_matrix, thresh):
    ...
    if _HAS_LAP:
        ...
        keep_mask = selected_costs <= thresh
        row_ind = row_ind[keep_mask]
        col_ind = col_ind[keep_mask]
        if row_ind.size > 0:
            matches = np.stack([row_ind, col_ind], axis=1)
        else:
            matches = np.empty((0, 2), dtype=int)
        unmatched_a = tuple(set(range(cost_matrix.shape[0])) - set(row_ind.tolist()))
        unmatched_b = tuple(set(range(cost_matrix.shape[1])) - set(col_ind.tolist()))
        return matches, unmatched_a, unmatched_b
